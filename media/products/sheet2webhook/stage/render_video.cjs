...
const FINAL_OUTPUT_MP4 = path.join(VIDEO_DIR, 'sheet2webhook-short.mp4');
const TOTAL_DURATION_SEC = 43.41;

const DAM_DIR = path.resolve('C:/dev/864zeros-publish/media/products/sheet2webhook');
...
(async () => {
    ...
    console.log(`Recording animation for ${waitMs}ms...`);
    ...
    // Milestone verification test captures (3s, 10s, 20s, 28s, 38s)
    console.log('--- Step 4b: Extracting 5-point milestone verification frames ---');
    const milestoneSecs = [3, 10, 20, 28, 38];
    for (const sec of milestoneSecs) {
        const frameFile = path.join(VIDEO_DIR, `frame_${sec}s.png`);
        execSync(`ffmpeg -y -ss ${sec}.0 -i "${FINAL_OUTPUT_MP4}" -frames:v 1 -q:v 2 "${frameFile}"`, { stdio: 'ignore' });
        console.log(`Milestone frame extracted: ${frameFile}`);
    }

    // Distribute to DAM and Web
    console.log('--- Step 5: Distributing to DAM, Web, and Artifacts ---');
    fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(DAM_VIDEO_DIR, 'sheet2webhook-short.mp4'));
    fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(DAM_VIDEO_DIR, 'sheet2webhook-master.mp4'));
    fs.copyFileSync(thumbFile, path.join(DAM_VIDEO_DIR, 'sheet2webhook-thumbnail.png'));
    ...
    if (fs.existsSync(WEB_ASSETS_DIR)) {
        fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(WEB_ASSETS_DIR, 'sheet2webhook-short.mp4'));
        fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(WEB_ASSETS_DIR, 'sheet2webhook-master.mp4'));
        fs.copyFileSync(thumbFile, path.join(WEB_ASSETS_DIR, 'sheet2webhook-thumbnail.png'));
    }

    if (fs.existsSync(ARTIFACT_DIR)) {
        fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(ARTIFACT_DIR, 'sheet2webhook-short.mp4'));
        fs.copyFileSync(thumbFile, path.join(ARTIFACT_DIR, 'sheet2webhook-thumbnail.png'));
    }
