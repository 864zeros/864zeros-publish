...
const FINAL_OUTPUT_MP4 = path.join(VIDEO_DIR, 'mailsyncshield-short.mp4');

const DAM_DIR = path.resolve('C:/dev/864zeros-publish/media/products/mailsyncshield');
...
(async () => {
    ...
    // Extract 5 milestone frames for verification gate
    const MILESTONES = [3, 10, 20, 28, 38];
    for (const sec of MILESTONES) {
        const framePath = path.join(VIDEO_DIR, `frame_${sec}s.png`);
        execSync(`ffmpeg -y -ss ${sec} -i "${FINAL_OUTPUT_MP4}" -frames:v 1 -q:v 2 "${framePath}"`, { stdio: 'ignore' });
        console.log(`Milestone frame extracted: ${framePath}`);
    }

    // Distribute to DAM and Web
    console.log('--- Step 5: Distributing to DAM, Web, and Artifacts ---');
    fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(DAM_VIDEO_DIR, 'mailsyncshield-short.mp4'));
    fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(DAM_VIDEO_DIR, 'mailsyncshield-master.mp4'));
    fs.copyFileSync(thumbFile, path.join(DAM_VIDEO_DIR, 'mailsyncshield-thumbnail.png'));
    ...
    if (fs.existsSync(WEB_ASSETS_DIR)) {
        fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(WEB_ASSETS_DIR, 'mailsyncshield-short.mp4'));
        fs.copyFileSync(thumbFile, path.join(WEB_ASSETS_DIR, 'mailsyncshield-thumbnail.png'));
    }

    if (fs.existsSync(ARTIFACT_DIR)) {
        fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(ARTIFACT_DIR, 'mailsyncshield-short.mp4'));
        fs.copyFileSync(thumbFile, path.join(ARTIFACT_DIR, 'mailsyncshield-thumbnail.png'));
    }
