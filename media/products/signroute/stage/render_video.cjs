...
const FINAL_OUTPUT_MP4 = path.join(VIDEO_DIR, 'signroute-short.mp4');
const TOTAL_DURATION_SEC = 44.61;

const DAM_DIR = path.resolve('C:/dev/864zeros-publish/media/products/signroute');
...
(async () => {
    ...
    for (const sec of milestoneSecs) {
        ...
        execSync(`ffmpeg -y -i "${FINAL_OUTPUT_MP4}" -ss ${sec}.0 -frames:v 1 -q:v 2 "${frameFile}"`, { stdio: 'ignore' });
    ...
    fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(DAM_VIDEO_DIR, 'signroute-short.mp4'));
    fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(DAM_VIDEO_DIR, 'signroute-master.mp4'));
    fs.copyFileSync(thumbFile, path.join(DAM_VIDEO_DIR, 'signroute-thumbnail.png'));
    ...
    if (fs.existsSync(WEB_ASSETS_DIR)) {
        fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(WEB_ASSETS_DIR, 'signroute-short.mp4'));
        fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(WEB_ASSETS_DIR, 'signroute-master.mp4'));
        fs.copyFileSync(thumbFile, path.join(WEB_ASSETS_DIR, 'signroute-thumbnail.png'));
    }

    if (fs.existsSync(ARTIFACT_DIR)) {
        fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(ARTIFACT_DIR, 'signroute-short.mp4'));
        fs.copyFileSync(thumbFile, path.join(ARTIFACT_DIR, 'signroute-thumbnail.png'));
    }
