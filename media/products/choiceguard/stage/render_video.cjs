...
const FINAL_OUTPUT_MP4 = path.join(VIDEO_DIR, 'choiceguard-short.mp4');
...
(async () => {
    ...
    console.log(`Thumbnail poster extracted at: ${thumbFile}`);

    // Vaulting to Central DAM and Brain Artifacts
    console.log('--- Step 5: Vaulting assets to Central DAM & Artifact directory ---');
    fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(DAM_VIDEO_DIR, 'choiceguard-short.mp4'));
    fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(DAM_VIDEO_DIR, 'choiceguard-master.mp4'));
    fs.copyFileSync(thumbFile, path.join(DAM_VIDEO_DIR, 'choiceguard-thumbnail.png'));
    fs.copyFileSync(SOUNDTRACK_FILE, path.join(DAM_AUDIO_DIR, 'soundtrack.wav'));
    fs.copyFileSync(path.join(VIDEO_DIR, 'audio', 'voiceover_full.wav'), path.join(DAM_AUDIO_DIR, 'voiceover_full.wav'));
    fs.copyFileSync(path.join(VIDEO_DIR, 'audio', 'timeline.json'), path.join(DAM_AUDIO_DIR, 'timeline.json'));
    fs.copyFileSync(HTML_FILE, path.join(DAM_STAGE_DIR, 'video.html'));

    if (fs.existsSync(ARTIFACT_DIR)) {
        fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(ARTIFACT_DIR, 'choiceguard-short.mp4'));
        fs.copyFileSync(thumbFile, path.join(ARTIFACT_DIR, 'choiceguard-thumbnail.png'));
    }

    // Clean up temp recording
    fs.rmSync(TEMP_RECORD_DIR, { recursive: true, force: true });
    console.log('✅ ChoiceGuard Master Explainer successfully rendered, vaulted, and cataloged!');
})();
