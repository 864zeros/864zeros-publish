...
const FINAL_OUTPUT_MP4 = path.join(VIDEO_DIR, 'slidebuilder-short.mp4');

const DAM_DIR = path.resolve('C:/dev/864zeros-publish/media/products/slidebuilder');
...
(async () => {
    ...
    fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(DAM_VIDEO_DIR, 'slidebuilder-short.mp4'));
    fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(DAM_VIDEO_DIR, 'slidebuilder-master.mp4'));
    fs.copyFileSync(thumbFile, path.join(DAM_VIDEO_DIR, 'slidebuilder-thumbnail.png'));
    ...
    if (fs.existsSync(WEB_ASSETS_DIR)) {
        fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(WEB_ASSETS_DIR, 'slidebuilder-short.mp4'));
        fs.copyFileSync(thumbFile, path.join(WEB_ASSETS_DIR, 'slidebuilder-thumbnail.png'));
    }

    if (fs.existsSync(ARTIFACT_DIR)) {
        fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(ARTIFACT_DIR, 'slidebuilder-short.mp4'));
        fs.copyFileSync(thumbFile, path.join(ARTIFACT_DIR, 'slidebuilder-thumbnail.png'));
    }
