...
const FINAL_OUTPUT_MP4 = path.join(VIDEO_DIR, 'approvalworkflow-short.mp4');

const DAM_DIR = path.resolve('C:/dev/864zeros-publish/media/products/approvalworkflow');
...
(async () => {
    ...
    fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(DAM_VIDEO_DIR, 'approvalworkflow-short.mp4'));
    fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(DAM_VIDEO_DIR, 'approvalworkflow-master.mp4'));
    fs.copyFileSync(thumbFile, path.join(DAM_VIDEO_DIR, 'approvalworkflow-thumbnail.png'));
    ...
    if (fs.existsSync(WEB_ASSETS_DIR)) {
        fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(WEB_ASSETS_DIR, 'approvalworkflow-short.mp4'));
        fs.copyFileSync(thumbFile, path.join(WEB_ASSETS_DIR, 'approvalworkflow-thumbnail.png'));
    }

    if (fs.existsSync(ARTIFACT_DIR)) {
        fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(ARTIFACT_DIR, 'approvalworkflow-short.mp4'));
        fs.copyFileSync(thumbFile, path.join(ARTIFACT_DIR, 'approvalworkflow-thumbnail.png'));
    }
