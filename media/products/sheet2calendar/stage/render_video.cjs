...
const FINAL_OUTPUT_MP4 = path.join(VIDEO_DIR, 'sheet2calendar-short.mp4');
const TOTAL_DURATION_SEC = 41.95;

const DAM_DIR = path.resolve('C:/dev/864zeros-publish/media/products/sheet2calendar');
...
(async () => {
    ...
    fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(DAM_VIDEO_DIR, 'sheet2calendar-short.mp4'));
    fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(DAM_VIDEO_DIR, 'sheet2calendar-master.mp4'));
    fs.copyFileSync(thumbFile, path.join(DAM_VIDEO_DIR, 'sheet2calendar-thumbnail.png'));
    ...
    if (fs.existsSync(WEB_ASSETS_DIR)) {
        fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(WEB_ASSETS_DIR, 'sheet2calendar-short.mp4'));
        fs.copyFileSync(thumbFile, path.join(WEB_ASSETS_DIR, 'sheet2calendar-thumbnail.png'));
    }

    if (fs.existsSync(ARTIFACT_DIR)) {
        fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(ARTIFACT_DIR, 'sheet2calendar-short.mp4'));
        fs.copyFileSync(thumbFile, path.join(ARTIFACT_DIR, 'sheet2calendar-thumbnail.png'));
    }
