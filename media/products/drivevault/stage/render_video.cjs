...
const FINAL_OUTPUT_MP4 = path.join(VIDEO_DIR, 'drivevault-short.mp4');

const DAM_DIR = path.resolve('C:/dev/864zeros-publish/media/products/drivevault');
...
(async () => {
    ...
    fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(DAM_VIDEO_DIR, 'drivevault-short.mp4'));
    fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(DAM_VIDEO_DIR, 'drivevault-master.mp4'));
    fs.copyFileSync(thumbFile, path.join(DAM_VIDEO_DIR, 'drivevault-thumbnail.png'));
    ...
    if (fs.existsSync(WEB_ASSETS_DIR)) {
        fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(WEB_ASSETS_DIR, 'drivevault-short.mp4'));
        fs.copyFileSync(thumbFile, path.join(WEB_ASSETS_DIR, 'drivevault-thumbnail.png'));
    }

    if (fs.existsSync(ARTIFACT_DIR)) {
        fs.copyFileSync(FINAL_OUTPUT_MP4, path.join(ARTIFACT_DIR, 'drivevault-short.mp4'));
        fs.copyFileSync(thumbFile, path.join(ARTIFACT_DIR, 'drivevault-thumbnail.png'));
    }
